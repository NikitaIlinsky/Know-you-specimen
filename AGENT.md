...
The `/src/know_your_specimen/` package contains: main.py (CLI), server.py (FastAPI), config.py, and subpackages classification/, initialization/, report/, segmentation/.

### 3.2. Test Directory Structure
All tests in `/test/` with `test_*.py` naming: test_main.py (CLI tests), test_server.py (API integration tests).
...
- `POST /api/v1/analyze` — multipart file upload, returns JSON with stats + artifact URLs.
- `GET /api/v1/output/{filename}` — serves generated artifact files (images, JSON).
### 6.2. Request/Response Models
- **Request**: `multipart/form-data` with a single `file` field (image upload).
- **Response (200)**: JSON with `stats` dict and `artifacts` object (URL strings for annotated image, mask, stats JSON).
- **Error**: JSON `{"detail": "..."}` with appropriate status code.
### 6.3. Status Codes
- `200` — image processed successfully.
- `404` — artifact file not found.
- `422` — validation error (missing file, unsupported extension, unreadable image).
- `500` — internal processing failure.
### 6.4. Versioning Strategy
API endpoints are namespaced under `/api/v1/`. Future breaking changes use `/api/v2/`.
...
### 7.1. Test Framework
pytest (configured via `pytest.ini`). Run with `uv run pytest`.

### 7.2. Test Types (Unit, Integration, E2E)
Integration tests via FastAPI `TestClient`. No E2E or pure unit tests yet.
### 7.3. Test Directory Structure
All tests in `/test/`, files follow `test_*.py` naming.

### 7.4. Coverage Requirements
None configured.
### 7.5. Fixtures and Factories
pytest fixtures: `client` (TestClient with temp dir), `valid_image_bytes` (synthetic JPEG). Temp dirs via `tempfile.TemporaryDirectory`, auto-cleaned.

### 7.6. Mocking Strategy
`unittest.mock.patch` for `process_file` in error-path tests. Happy-path tests use real pipeline.

### 7.7. Test Data Management
Synthetic images generated in-memory via numpy + cv2. No external fixtures needed.
